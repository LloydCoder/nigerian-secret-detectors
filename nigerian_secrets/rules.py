...
from .providers import PROVIDERS, DetectionType
...
BASE_RULES: tuple[Rule, ...] = (
    ...
    Rule(
        ...
        "Monnify secret credential pattern detected.", "provider-specific",
    ),
    Rule(
        "korapay-secret-key", "korapay", "fintech", "critical",
        re.compile(r"\b(?:sk_(?:live|test)_|pk_(?:live|test)_)[A-Za-z0-9_-]{20,}\b", re.I),
        ("korapay", "kora pay"),
        "KoraPay credential pattern detected; provider context is required for high confidence.", "provider-specific",
    ),
    Rule(
        ...
        "SeerBit credential-like value detected in provider context.", "provider-context",
    ),
    Rule(
        ...
        "Interswitch WebPAY MAC key pattern detected.", "provider-specific",
    ),
    Rule(
        ...
        "Remita credential-like value detected in provider context.", "provider-context",
    ),
    Rule(
        ...
        ("opay", "o-pay"),
        "OPay credential-like value detected in provider context.", "provider-context",
    ),
    Rule(
        "palmpay-secret-context", "palmpay", "fintech", "high",
        re.compile(r"\b(?:secret|api[_-]?key|private[_-]?key|access[_-]?token)\s*[:=]\s*[\"']([A-Za-z0-9+/=_-]{24,})[\"']", re.I),
        ("palmpay", "palm pay", "palm-pay"),
        "PalmPay credential-like value detected in provider context.", "provider-context",
    ),
    Rule(
        "generic-private-key", "crypto", "cryptographic", "critical",
        re.compile(r"-----BEGIN (?:RSA |EC |OPENSSH |PGP )?PRIVATE KEY-----"),
        (),
        "Private key material detected.", "cryptographic",
    ),
    Rule(
        ...
        "JWT-like token detected in Nigerian financial provider context.", "token",
    ),
)
